...
class HrPayslip(models.Model):
    ...
    employee_id = fields.Many2one('hr.employee', string='Employee', required=True, readonly=True,
                                  states={'draft': [('readonly', False)]})
    employee_state = fields.Selection(string='Employee Status', related='employee_id.state')
    ...
    actual_working_hours = fields.Float(string="Actual Working Hours", help="This value will be calculated using timesheet module")
    employee_company_id = fields.Many2one('hr.employee.company')

    @api.model_create_multi
    def create(self, vals_list):
        for values in vals_list:
            if ('employee_id' in values) and ('pay_period_id' in values):
                existing_payslip_count = self.search_count([('employee_id','=',values.get('employee_id')),('pay_period_id','=',values.get('pay_period_id'))])
                if existing_payslip_count > 0:
                    raise UserError(_("You should remove already created payslip (It might be some duplicated payslips!).")) 
                else:        
                    input_objs = self.env['hr.payslip.input'].search([('employee_id','=',values.get('employee_id')),('pay_period_id','=',values.get('pay_period_id'))])
                    if input_objs:
                        values['input_line_ids'] = input_objs
        return super(HrPayslip, self).create(vals_list)

    # @api.depends('employee_id','pay_period_id')
    # def _compute_input_line_ids(self):
    #     for payslip in self:
    #         if payslip.employee_id and payslip.pay_period_id:
    #             input_objs = payslip.env['hr.payslip.input'].search([('employee_id','=',payslip.employee_id.id),('pay_period_id','=',payslip.pay_period_id.id)])
    #             print('input_objs*************************', input_objs)
    #             if input_objs:
    #                 payslip.input_line_ids = input_objs
    ...
    def _compute_details_by_salary_rule_category(self):
        for payslip in self:
            payslip.details_by_salary_rule_category = payslip.mapped('line_ids').filtered(
                lambda line: line.category_id.id)
    ...
    def accounting_sheet(self):
        return self.write({'state': 'accounting'})

    def action_payslip_done(self):
        if not self.env.context.get('without_compute_sheet'):
            existing_payslip_count = self.env['hr.payslip'].search_count([('employee_id','=',self.employee_id.id),('pay_period_id','=',self.pay_period_id.id)])
            if existing_payslip_count > 1:
               raise UserError(('There are an existing payslip for %s in selected pay period') % (self.employee_id.name, ))
            else:
                self.compute_sheet()
    ...
    def get_contract(self, employee, date_from, date_to):
        ...
        clause_1 = ['&', ('date_start', '<=', date_to), ('date_end', '>=', date_from)]
        # OR if it starts between the given dates
        # clause_2 = ['&', ('date_start', '<=', date_to), ('date_start', '>=', date_from)]
        # OR if it starts before the date_from and finish after the date_end (or never finish)
        # clause_3 = ['&', ('date_start', '<=', date_from), '|', ('date_end', '=', False), ('date_end', '>=', date_to)]
        clause_final = [('employee_id', '=', employee.id), ('state', 'in', ('open','onleave'))] + clause_1
        contract_ids = False
    ...
    def onchange_employee_id(self, date_from, date_to, employee_id=False, contract_id=False):
        ...
        if not self.env.context.get('contract'):
            ...
            if contract_id:
                ...
                # need to add conditions to get_contract ********

        if not contract_ids:
            return res
        contract = self.env['hr.contract'].browse(contract_ids)[-1]
        res['value'].update({
            'contract_id': contract.id
        })
    ...
    def onchange_pay_period(self):
        if self.pay_period_id:
            self.date_from = self.pay_period_id.start_date
            self.date_to = self.pay_period_id.end_date
...
class HrPayslipInput(models.Model):
    ...
    name = fields.Char(string='Description', required=True)
    payslip_id = fields.Many2one('hr.payslip', string='Pay Slip', ondelete='cascade', index=True)
    sequence = fields.Integer(required=True, index=True, default=10)
    code = fields.Char(required=True, help="The code that can be used in the salary rules")
    ...
    employee_id = fields.Many2one('hr.employee', string='Employee', required=True)
    pay_period_id = fields.Many2one('hr.pay.period', "Pay Period")
    is_grossable = fields.Boolean(string="Is Grossable")
...
class HrPayslipRun(models.Model):
    ...
    name = fields.Char(required=True, readonly=True, tracking=True, states={'draft': [('readonly', False)]})
    slip_ids = fields.One2many('hr.payslip', 'payslip_run_id', string='Payslips', readonly=True, tracking=True,
                               states={'draft': [('readonly', False)]})
    state = fields.Selection([
        ('draft', 'Draft'),
        ('close', 'Close'),
    ...
    date_start = fields.Date(string='Date From', required=True, readonly=True,
                             states={'draft': [('readonly', False)]},
                             related='pay_period_id.start_date',)
    date_end = fields.Date(string='Date To', required=True, readonly=True,
                           states={'draft': [('readonly', False)]},
                           related='pay_period_id.end_date',
                           )
    credit_note = fields.Boolean(string='Credit Note', readonly=True,
                                 states={'draft': [('readonly', False)]},
                                 help="If its checked, indicates that all payslips generated from here are refund payslips.")

    pay_period_id = fields.Many2one('hr.pay.period', string='Pay Period', tracking=True,)
    company_id = fields.Many2one('res.company', string='Company', readonly=True, copy=False, tracking=True,
                                 default='your company',
                                 states={'draft': [('readonly', False)]})
    employee_company_id = fields.Many2one('hr.employee.company')
    reference_no = fields.Char(string='Batch Reference', readonly=True,
                               default=lambda self: _('New'), )
    struct_ids = fields.Many2many('hr.payroll.structure', 'hr_payslipbatch_structure_rel', 'payslip_batch_id',
                                  'struct_id', string='Salary Structure', tracking=True, readonly=True,
                                  states={'draft': [('readonly', False)]})

    @api.model_create_multi
    def create(self, vals_list):
        for val in vals_list:
            if val.get('reference_no', _('New')) == _('New'):
                val['reference_no'] = self.env['ir.sequence'].next_by_code('hr.payslip.run.sequence')
    ...
    # by closing  batch, all including payslips are going to be approved
    def close_payslip_run(self):
        for rec in self:
            if rec.slip_ids:
                for payslip_obj in rec.slip_ids:
    ...
    def unlink(self):
        for rec in self:
            if rec.state == 'close':
                raise UserError('You are not allowed to delete Batchslip in Close state')
        return super().unlink()
