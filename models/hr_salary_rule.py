...
class HrPayrollStructure(models.Model):
    ...
    company_id = fields.Many2one('res.company', string='Company', required=True,
                                 copy=False, default='your company')
    ...
    insurance_profile_id = fields.Many2one('hr.insurance.profile', 'Insurance Profile')
    maximum_working_hours = fields.Integer(string="Maximum Working Hours", default=0)
...
class HrSalaryRule(models.Model):
    ...
    # @api.depends('name','struct_id')
    # def _compute_code(self):
    #     for rule in self:
    #         if rule.name and rule.struct_id:
    #             rule.code = str(rule.name) + '-' + str(rule.struct_id.name)
...
class HrInsuranceProfile(models.Model):
    _name = 'hr.ins.profile'
    _description = 'Insurance Profile'
    _inherit = ['mail.thread','mail.activity.mixin']

    name = fields.Char('Name')
    max_insurable_gross = fields.Float(string='Maximum Insurable Gross')
    employee_rate = fields.Float(string='Employee Insurance Rate', default=0.07)
    employer_rate = fields.Float(string='Employer Insurance Rate', default=0.2)
    unemployment_rate = fields.Float(string='Unemployment Insurance Rate', default=0.03)
    low_limit = fields.Float(string='Low Limit', help="Minimum Full Day Absence")
    max_limit = fields.Float(string='Max Limit', help="Max  Full Day Absence")
